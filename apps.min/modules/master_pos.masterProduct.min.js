...
form.findField('product_bg_color_show').setValue(me.edit_data.product_bg_color);form.findField('product_bg_color_show').inputEl.setStyle({backgroundColor:'#'+me.edit_data.product_bg_color,border:'1px solid #ccc',textAlign:'center'});form.findField('product_text_color_show').setValue(me.edit_data.product_text_color);form.findField('product_text_color_show').inputEl.setStyle({backgroundColor:'#'+me.edit_data.product_text_color,border:'1px solid #ccc',textAlign:'center'});Ext.getCmp('product_priceqty_list').setDisabled(false);Ext.getCmp('grid_masterProductPriceQty').store.proxy.extraParams.product_id=me.edit_data.id;Ext.getCmp('grid_masterProductPriceQty').store.load();}else{form.reset();form.findField('qty_unit').setValue(1);form.findField('is_active').setValue(1);form.findField('product_type').setValue('item');Ext.getCmp('product_package_list').setDisabled(true);Ext.getCmp('product_gramasi_list').setDisabled(true);Ext.getCmp('product_varian_list').setDisabled(true);Ext.getCmp('product_priceqty_list').setDisabled(true);Ext.getCmp('btnUpdateProductCode_masterProduct').hide();form.findField('product_bg_color_show').setValue('000000');form.findField('product_bg_color_show').inputEl.setStyle({backgroundColor:'#000000',border:'1px solid #ccc',textAlign:'center'});form.findField('product_text_color_show').setValue('FFFFFF');form.findField('product_text_color_show').inputEl.setStyle({backgroundColor:'#FFFFFF',border:'1px solid #ccc',textAlign:'center'});}
...
myMask.hide();Ext.getCmp('varian_id_masterProductVarian').select(varian_id);}});}}});},add_masterProductGramasi:function(me){var theApp=me.app;var desktop=theApp.getDesktop();var Titletext='Add Link Item ';if(me.formType_listGramasi=='edit'){Titletext='Update Link Item: ';}else{me.edit_data_listGramasi=[];}
...
myMask.hide();}});me.showhide_productVarian(1,"Package");}}});},add_masterProductPriceQty:function(me){var theApp=me.app;var desktop=theApp.getDesktop();var Titletext='Add Price/Qty ';if(me.formType_listPriceQty=='edit'){Titletext='Update Price/Qty ';}else{me.edit_data_listPriceQty=[];}
return desktop.createWindow({id:me.id+'_add_masterProductPriceQty',title:Titletext,width:350,height:215,iconCls:'btn-add',animCollapse:false,constrainHeader:true,resizable:false,minimizable:false,maximizable:false,modal:true,border:0,layout:{type:'fit'},items:[{xtype:'form',id:'form_masterProductPriceQty',border:0,bodyPadding:10,margin:'0 0 0 0',items:[{xtype:'hidden',name:'form_type_masterProductPriceQty',value:me.formType_listPriceQty},{xtype:'hidden',name:'product_id'},{xtype:'hidden',name:'product_varian_id'},{xtype:'hidden',name:'varian_id'},{xtype:'hidden',name:'has_varian'},{xtype:'hidden',name:'id'},{xtype:'hidden',name:'product_price_default'},{xtype:'textfield',name:'qty_from',fieldLabel:'Qty Min',width:170,labelWidth:100,allowBlank:false},{xtype:'textfield',name:'qty_till',fieldLabel:'Qty Max',width:170,labelWidth:100,allowBlank:false},{xtype:'textfield',name:'product_price',fieldLabel:'Hrg.Jual',width:250,labelWidth:100,allowBlank:false}]}],buttons:[{text:'Save',formBind:true,id:'btnSave_masterProductPriceQty',iconCls:'btn-save',handler:function(){me.save_masterProductPriceQty();}},{text:'Cancel',iconCls:'btn-cancel',handler:function(){me.doClose(me.id+'_add_masterProductPriceQty');}}],listeners:{show:function(){var form=Ext.getCmp('form_masterProductPriceQty').getForm();var myMask=new Ext.LoadMask(Ext.getCmp('form_masterProductPriceQty'),{msg:"Loading..."});myMask.show();if(me.formType_listPriceQty=='edit'){form.setValues(me.edit_data_listPriceQty);}else{form.reset();form.findField('product_price').setValue(me.edit_data.product_price);}
form.findField('product_price_default').setValue(me.edit_data.product_price);var default_product_varian_id_masterProductPackage=Ext.getCmp("default_product_varian_id_masterProductPriceQty").getValue();var default_varian_id_masterProductPackage=Ext.getCmp("default_varian_id_masterProductPriceQty").getValue();form.findField('product_varian_id').setValue(default_product_varian_id_masterProductPackage);form.findField('varian_id').setValue(default_varian_id_masterProductPackage);form.findField('product_id').setValue(me.edit_data.id);myMask.hide();me.showhide_productVarian(1,"PriceQty");}}});},add_uploadMasterProduct:function(me){var theApp=me.app;var desktop=theApp.getDesktop();var Titletext='Upload Product ';if(me.formType=='edit'){Titletext='Upload Product: ';}
