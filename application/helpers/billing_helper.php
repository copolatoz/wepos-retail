...
if(!function_exists('getBilling')){
	function getBilling($billing_id = ''){
		...
		'jumlah_shift','shift_active','default_tipe_billing_so');
		...
		$is_salesorder = 0;
		if(!empty($get_opt['default_tipe_billing_so'])){
			if($get_opt['default_tipe_billing_so'] == $table_id){
				$is_salesorder = 1;
			}
		}
		...
		if(empty($billing_id)){
			...
			//update-2011.001
			$trx_mk = strtotime("now");
			$billing_date = date('Y-m-d',$trx_mk);
			$billing_datetime = date('Y-m-d H:i:s',$trx_mk);
			$billing_time = date('H:i',$trx_mk);
			...
			{  
				...
				//update-2011.001
				if(!empty($is_salesorder) AND !empty($billing_id)){
					
					$so_number = generate_so_number($trx_mk, $get_no_billing);
					$update_billing = array(
						'is_salesorder'		=> $is_salesorder,
						'billing_date'		=> $billing_date,
						'billing_datetime'	=> $billing_datetime,
						'billing_time'		=> $billing_time,
						'billing_no'		=> $so_number,
						'billing_no_before'	=> $get_no_billing,
					);
					
					$scope->db->update($scope->table,$update_billing,"id = ".$billing_id);
		...
		$scope->db->select('a.id, a.table_id, a.table_no, a.billing_no, a.payment_date,
			...
			a.id as billing_id, a.voucher_no, a.is_sistem_tawar, a.single_rate, a.is_salesorder,
			a.txmark, a.txmark_no, a.billing_date, a.billing_datetime, a.billing_time, a.billing_no_before,
		...
		if($get_last->num_rows() > 0){
			...
			//update-2010.001
			if(!empty($billingData->is_salesorder)){
				$billingData->billing_no_show = 'S'.$billingData->billing_no;
			}
...
if(!function_exists('generate_billing_no')){
	function generate_billing_no(){
		...
		$scope->db->select("id,billing_no,billing_no_before,is_salesorder");
		$scope->db->from($scope->table);
		$scope->db->where("(billing_no LIKE '".$billing_date."%' OR (billing_no_before LIKE '".$billing_date."%' AND is_salesorder = 1))");
		//$scope->db->where("billing_no LIKE '".$billing_date."%'");
		//$scope->db->order_by('billing_no', 'DESC');
		...
		if($get_last->num_rows() > 0){
			...
			if(!empty($data_billing->is_salesorder)){
				$billing_no = $data_billing->billing_no_before;
			}
...
if(!function_exists('updateTable')){	
	...
	function updateTable($data_create = array(), $billingData = array()){
		...
		$table_id = $scope->input->post('table_id', true);
		$table_no = $scope->input->post('table_no', true);
		...
		//update-2010.001
		$input_tanggal_manual_so = $scope->input->post('input_tanggal_manual_so', true);
		$tanggal_so = $scope->input->post('tanggal_so', true);
		$jam_so = $scope->input->post('jam_so', true);
		$is_salesorder = $scope->input->post('is_salesorder', true);
		$is_salesorder_before = $scope->input->post('is_salesorder_before', true);
		...
		if(!empty($billingData)){
			...
			$is_salesorder = $billingData->is_salesorder;
			$is_salesorder_before = $billingData->is_salesorder;
		}else{
			$billingData = getBilling($billing_id);	
			$billingData->table_id = $table_id;
			$billingData->table_no = $table_no;
			$billingData->is_salesorder = $is_salesorder;
		}
		
		if(empty($billing_id)){
			$r = array('success' => false, 'info' => 'Silahkan Pilih Tipe Billing!');
		}else{
			...
			if(!empty($is_delete)){
				...
				if(!empty($is_salesorder)){
					$data_update_billing['is_salesorder'] = 1;
				}else{
					//walkin
					$data_update_billing['is_salesorder'] = 0;
					if(!empty($is_salesorder_before)){
						$rollback_so = 1;
					}
				}
				
				if($rollback_so == 0){
					if(!empty($input_tanggal_manual_so)){
						if(empty($tanggal_so)){
						...
						if(empty($jam_so)){
							...
							$jam_so_exp = explode(":", $jam_so);
							
							if(empty($jam_so_exp[0])){
								$jam_so_exp[0] = date("H");
							}
							
							if(strlen($jam_so_exp[0]) < 2){
								$jam_so_exp[0] = '0'.$jam_so_exp[0];
							}
							
							if(empty($jam_so_exp[1])){
								$jam_so_exp[1] = date("i");
							}
							
							if(strlen($jam_so_exp[1]) < 2){
								$jam_so_exp[1] = '0'.$jam_so_exp[1];
							}
							
							$jam_so = $jam_so_exp[0].":".$jam_so_exp[1].":00";
						}
						
						$trx_date_mk = strtotime($tanggal_so." ".$jam_so);
						
						$is_backdate = 0;
						if($tanggal_so != date("d-m-Y")){
							//backdate
							$is_backdate = 1;
							
							//CLOSING DATE
							$var_closing = array(
								'xdate'	=> date("Y-m-d H:i:s", $trx_date_mk),
								'xtipe'	=> 'sales'
							);
							$is_closing = is_closing($var_closing);
							if($is_closing){
						...
						//get SO number
						$so_number = generate_so_number($trx_date_mk, $billingData->billing_no);
						if(empty($billingData->billing_no_before)){
							$data_update_billing['billing_no_before'] = $billingData->billing_no;
						}
						
						$data_update_billing['billing_no'] = $so_number;
						$data_update_billing['billing_date'] = date("Y-m-d", $trx_date_mk);
						$data_update_billing['billing_datetime'] = date("Y-m-d H:i:s", $trx_date_mk);
						$data_update_billing['billing_time'] = date("H:i", $trx_date_mk);
						
						
						$billingData->billing_no = $so_number;
						$billingData->billing_no_show = $so_number;
						$billingData->billing_date = date("Y-m-d", $trx_date_mk);
						$billingData->billing_datetime = date("Y-m-d H:i:s", $trx_date_mk);
						$billingData->billing_time = date("H:i", $trx_date_mk);
					}else{
						
						$trx_mk = strtotime($billingData->created);
						$data_update_billing['billing_date'] = date("Y-m-d", $trx_mk);
						$data_update_billing['billing_datetime'] = date("Y-m-d H:i:s", $trx_mk);
						$data_update_billing['billing_time'] = date("H:i", $trx_mk);
						
						$date_time_today = date("Y-m-d H:i:s");
						$data_update_billing['updated'] = $date_time_today;
						$data_update_billing['updatedby'] = $session_user;
						
						$billingData->billing_no = $billingData->billing_no;
						$billingData->billing_no_show = $billingData->billing_no;
						$billingData->billing_date = date("Y-m-d", $trx_mk);
						$billingData->billing_datetime = date("Y-m-d H:i:s", $trx_mk);
						$billingData->billing_time = date("H:i", $trx_mk);
					}	
				}
				
				if($rollback_so == 1){
					$data_update_billing['billing_no'] = $billingData->billing_no_before;
					$data_update_billing['billing_no_before'] = '';
					...
					$billingData->billing_no = $billingData->billing_no_before;
					$billingData->billing_no_show = $billingData->billing_no_before;
					$billingData->billing_date = date("Y-m-d", $trx_mk);
					$billingData->billing_datetime = date("Y-m-d H:i:s", $trx_mk);
					$billingData->billing_time = date("H:i", $trx_mk);
				}
				...
				if((!empty($input_tanggal_manual_so) AND $rollback_so == 0) OR $rollback_so == 1){
...
if(!function_exists('getBillingDetailCounter')){	
	
	function getBillingDetailCounter(){
		...
		$scope->table = $scope->prefix.'billing_detail';	
		$session_user = $scope->session->userdata('user_username');					
		...
		$scope->db->select("order_counter");
		$scope->db->from($scope->table);
		$scope->db->where('order_day_counter', $dateToday);
		$scope->db->order_by('order_counter', 'DESC');
		$get_last = $scope->db->get();
		if($get_last->num_rows() > 0){
...
if(!function_exists('generate_so_number')){
	
	function generate_so_number($trx_date = '', $billing_no = ''){
		...
		$default_PRD = "SO".date("ym", $trx_date);
		$scope->db->from($scope->table);
		$scope->db->where("is_salesorder = 1 AND billing_no LIKE '".$default_PRD."%' AND billing_no != '".$billing_no."'");
		$scope->db->order_by('billing_no', 'DESC');
		$get_last = $scope->db->get();
		if($get_last->num_rows() > 0){
			$data_so = $get_last->row();
			//$so_number = $data_so->so_number;
			$so_number = str_replace($default_PRD,"", $data_so->billing_no);
