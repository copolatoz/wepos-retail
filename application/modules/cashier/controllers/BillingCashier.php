...
class BillingCashier extends MY_Controller {
	...
	public function cancelBilling($is_paid = false){
		...
		if($get_billing->num_rows() > 0){
			...
			//	$r = array('success' => false, 'info' => 'Billing: '.$billingData->billing_no.' sudah dibayar!<br/>Cannot Cancel Billing, Please Refresh List Billing'); 
			//	echo json_encode($r);
			//	die();
			//}
		}else{
	...
	public function holdBilling($is_cancel = ''){
		...
		if($is_cancel == 'cancel'){
			...
			if($get_billing->num_rows() > 0){
				...
				if($billingData->billing_status == 'paid'){
					$r = array('success' => false, 'info' => 'Billing: '.$billingData->billing_no.' sudah dibayar!<br/>Cannot Hold Billing, Please Refresh List Billing'); 
					echo json_encode($r);
					die();
				}
	...
	public function cancelOrder(){
		...
		if($get_billing->num_rows() > 0){
			...
			if($billingData->package_item == 1 AND $billingData->free_item == 1 AND !empty($billingData->ref_order_id)){
				$r = array('success' => false, 'info' => 'Product/Item termasuk dalam Paket!<br/>Silahkan Cancel Menu/Product Utama Paket'); 
				echo json_encode($r);
				die();
			}
			
			if($billingData->package_item == 0 AND $billingData->free_item == 1 AND !empty($billingData->ref_order_id)){
				$r = array('success' => false, 'info' => 'Product/Item termasuk dalam Promo<br/>Please Cancel Menu/Product Utama'); 
				echo json_encode($r);
				die();
			}

			//$r = array('success' => false, 'info' => 'Billing: '.$billingData->billing_no.' sudah dibayar!<br/>Cannot Cancel Order, Please Refresh List Billing');
			//die(json_encode($r));
		}
	...
	public function save_payBilling(){
		...
		b.is_salesorder, b.billing_date, b.billing_datetime, b.billing_time");
		$this->db->from($this->table." as b");
		...
		{  
			...
			if(!empty($retail_warehouse)){
				...
				if($autocut_stok_sales_to_usage == 1){
					...
					//update-2010.001
					if(!empty($billingData->is_salesorder) AND !empty($billingData->billing_date)){
					...
					if($autocut_stok_sales == 1){
						...
						//update-2010.001
						if(!empty($billingData->is_salesorder) AND !empty($billingData->billing_date)){
							$post_params['date'] = $billingData->billing_date;
						}
	...
	public function loadingSettingRetail(){
		...
		$this->mprint->loadingSettingRetail();
