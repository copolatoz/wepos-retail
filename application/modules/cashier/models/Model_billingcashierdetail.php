...
class Model_BillingCashierDetail extends DB_Model {
	...
	function billingDetail($billing_id = '', $retail_warehouse, $update_stok = ''){
		...
		if(!empty($billing_id) AND !empty($retail_warehouse)){
			...
			$dtInsert_kode_unik_log = array();
			$all_unik_kode = array();
			$all_unik_kode_hpp = array();
							...
							foreach($list_dt_kode as $kode_unik){
								if(!empty($kode_unik)){
									if(!in_array($kode_unik, $all_unik_kode)){
										...
										$all_unik_kode_hpp[$kode_unik] = array(
											'item_hpp' => $dtRow->product_price_hpp, 
											'item_sales' => $dtRow->product_price
										);
										
										$dtInsert_kode_unik_log[$kode_unik] = array(
											//"item_id" => $dt['id_ref_item'],
											"kode_unik" => $kode_unik,
											"ref_out" => $billing_no,
											"date_out" => $payment_date.' '.date("H:i:s"),
			...
			if($update_stok == 'update' OR $update_stok == 'rollback'){
				...
				if($update_stok == 'rollback'){
					...
					$this->db->where("ref_out = '".$billing_no."'");
					$getOldKodeUnik = $this->db->get();
					if($getOldKodeUnik->num_rows() > 0){
						foreach($getOldKodeUnik->result() as $dt){
							
							$get_product_price_hpp = $dt->item_hpp;
							$get_product_price = 0;
							if(!empty($all_unik_kode_hpp[$dt->kode_unik])){
								$get_hpp_unik_kode = $all_unik_kode_hpp[$dt->kode_unik];
								$get_product_price_hpp = $get_hpp_unik_kode['item_hpp'];
								$get_product_price = $get_hpp_unik_kode['item_sales'];
							}
				...
				if(!empty($dtInsert_kode_unik_log)){
					
					$dtInsert_kode_unik_log_bu = $dtInsert_kode_unik_log;
					$dtInsert_kode_unik_log = array();
					...
					if(!empty($all_unik_kode)){
						...
						$this->db->select("item_id, kode_unik");
						$this->db->from($this->prefix."item_kode_unik");
						$this->db->where("kode_unik IN ('".$all_unik_kode_sql."')");
						$getOldKodeUnik = $this->db->get();
						if($getOldKodeUnik->num_rows() > 0){
							
							foreach($getOldKodeUnik->result() as $dt){
								
								if(!empty($dtInsert_kode_unik_log_bu[$dt->kode_unik])){
									$data_new = $dtInsert_kode_unik_log_bu[$dt->kode_unik];
									$data_new["item_id"] = $dt->item_id;
									
									$dtInsert_kode_unik_log[] = $data_new;
								}
					...
					$this->db->insert_batch($this->prefix.'item_kode_unik_log', $dtInsert_kode_unik_log);
				}
