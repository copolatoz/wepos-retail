...
class MasterTableInv extends MY_Controller {
	...
	public function gridData()
	{
		...
		if($show_available == true OR ($purpose == 'tableList' OR $purpose == 'loadDataViewTable')){
			...
			$billnoSO = date("ym", $tanggalmk);
			$this->db->select('*');
			$this->db->from($this->billing);
			$this->db->where("(billing_no LIKE '".$billno."%' OR (billing_no_before LIKE '".$billno."%' AND is_salesorder = 1))  AND billing_status = 'hold' AND is_deleted = 0 AND table_id > 0");
			...
			foreach ($get_data['data'] as $s){
				...
				if($purpose == 'tableList'){
					...
					if($no_data == 0){
						...
						$backup_id = $s['id'];
						$s['id'] = 0;
						$s['table_info'] = '<div style="font-size:12px; margin:5px">Tipe yg dipilih:</div>';
						if(empty($floorplan_id)){
							$s['table_info'] .= '<div style="font-size:16px; margin:5px 0px 15px;"><b>Pilih Tipe</b></div>';
						}else{
							$s['table_info'] .= '<div style="font-size:20px; margin:5px 0px 15px;"><b>'.$floorplan_name.'</b></div>';
							$s['table_info'] .= '<div style="font-size:10px;">Klik u/ lihat tipe Lainnya</div>';
						}
		...
		//$get_data['data_billing'] = $data_billing;
